...
template<>
void cpu_permute<float>(float* A, const int* P, const long long int rows, const long long int cols, bool permute_rows) 
{
  bool Debug = false;
...
template <>
void cpu_permute<double>(double* a, const int* pvt,const long long int  rows,const long long int  cols, bool direction)
{
  double* temp = (double*)malloc(rows * cols * sizeof(double));
  checkErrors(temp);
  host_copy<double>(rows * cols, a, temp);
  int ind;
  for (int i = 0; i < rows; i++){
    ind = pvt[i];
    for (int j = 0; j < cols; j++){
      if(direction){
        a[i * cols +j] = temp[ind * cols + j];
      }else{
        a[ind * cols +j] = temp[i * cols + j];
      };
    };
  };
  free(temp);
...
void fillupMatrix(T_ELEM *A , int lda , int rows, int cols, int seed)
{
    ...
    for (int j = 0; j < cols; j++)
    {
        ...
        {
            sign = rand() % 2;
            A[i + lda*j ] = (T_ELEM) (((double)(((lda*i+j+seed) % 253)+1))/256.0, ((double)((((cols*i+j) + 123 + seed) % 253)+1))/256.0);
            if(sign==0){
                A[i + lda*j ] = A[i + lda*j ] * (T_ELEM)(-1);
            };
            /*
            if(i<5 && j==0){
                printf("A[%d + %d * %d] = %f \n", i, lda, j, A[i + lda*j ]);
...
template
float nextafter(const float b);

template
double nextafter(const double b);
...
template
void host_rng_uniform<float>(const long long int n, const float a, const float b,
                              float* r);

template
void host_rng_uniform<double>(const long long int n, const double a, const double b,
                               double* r);
...
template <>
std::string ToString<bool>(bool val)
{
...
template <>
std::string ToString<int>(int val)
{
    std::stringstream stream;
    stream << val;
    return stream.str();
...
template <>
std::string ToString<float>(float val)
{
    std::stringstream stream;
    stream << val;
    return stream.str();
...
template <>
std::string ToString<double>(double val)
{
    std::stringstream stream;
    stream << val;
    return stream.str();
...
template <>
void printPartialMatrices<float>(float * A, float * B, int rows, int cols , int ld)
{
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < cols; j++)
...
template <>
void printPartialMatrices<int>(int * A, int * B, int rows, int cols , int ld)
{
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < cols; j++)
...
template <>
void printPartialMtx<int>(int * A, int rows, int cols , int ld)
{
...
template <>
void printPartialMtx<float>(float * A, int rows, int cols , int ld)
{
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < cols; j++)
        {
            if (j<cols-1){ printf("%f , ",  A[j*ld +i]); }
            else{ printf("%f ;\n ",  A[j*ld +i]); }
        }
    }
}
...
template <>
void printPartialMtx<double>(double * A, int rows, int cols , int ld)
{
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < cols; j++)
...
template <>
void save_host_array_to_file<int>(const int* A_host, int count, std::string title){
...
template <>
void save_host_array_to_file<float>(const float* A_host, int count, std::string title){
...
template <>
void append_host_array_to_file<int>(const int* A_host, int count, std::string title){
...
template <>
void append_host_array_to_file<float>(const float* A_host, int count, std::string title){
...
template <>
void save_host_mtx_to_file<int>(const int* A_host, const long long int lda, const long long int  sda, std::string title){
  ...
  for (int i = 0; i < lda; i++){
      for (int j = 0; j < sda; j++){
          entries<<A_host[(long long int)i + (long long int)j * lda];
              entries<<", ";
      }
      entries<<"\r\n";
  }
...
template <>
void save_host_mtx_to_file<float>(const float* A_host, const long long int lda, const long long int  sda, std::string title){
...
template <>
void save_host_mtx_to_file<double>(const double* A_host, const long long int lda, const long long int  sda, std::string title){
    ...
    for(long long int row = 0; row < ratings_rows_training; row ++){
        for(long long int i = csr_format_ratingsMtx_userID_dev_training[row]; i < csr_format_ratingsMtx_userID_dev_training[row + 1]; i++){
            ...
            if(row_major_ordering){
              if(Debug) {
                LOG("num_entries_GA : "<< num_entries_GA);
                LOG("ratings_rows_training : "<< ratings_rows_training);
                LOG("ratings_cols_training : "<< ratings_cols_training);
...
template <>
void cpu_set_all<float>(float* x, const int N, float alpha)
{
  for(int i=0; i <N; i++) {
    //origin+x1+rows*y1
    x[i]=alpha;
