...
const long long int CUDA_NUM_BLOCKS = 65535;

// use 512 threads per block
const  long long int CUDA_NUM_THREADS = 512;
...
template<>
bool gpu_isBad<float>(const float* A, long long int size) 
{
  ...
  if (num_gpu_blocks > CUDA_NUM_BLOCKS){
    ...
    while (num_gpu_blocks > CUDA_NUM_BLOCKS){
      gpu_isBad_kernel<float><<<CUDA_NUM_BLOCKS, CUDA_NUM_THREADS>>>(A +spot , num_entries, isBad);
    ...
    gpu_isBad_kernel<float><<<num_gpu_blocks, CUDA_NUM_THREADS>>>(A + spot , size - spot, isBad);
  }else{
    gpu_isBad_kernel<float><<<num_gpu_blocks, CUDA_NUM_THREADS>>>(A , size, isBad);
  };
...
template<>
void print_gpu_mtx_entries<bool>(const bool* array, int lda, int sda, bool transpose)
{

    const bool *host_pointer;
    
    host_pointer=(const bool *)malloc(lda * sda * sizeof(bool));
    checkErrors(host_pointer);
    checkCudaErrors(cudaMemcpy((void*)host_pointer, array, lda * sda * sizeof(bool),cudaMemcpyDeviceToHost));
    ...
    if(!transpose){
      for( int j = 0; j < sda; j+= 1 ) {
        for( int i = 0; i < lda; i+= 1 ) {
          if (j==sda-1){
            if (i==lda-1){
              line = (line + ToString(host_pointer[i  + j* lda])).c_str();
            }else{
              line = (line + ToString(host_pointer[i  + j* lda]) + " ; ").c_str();
            };
          }else{
...
template<>
void print_gpu_mtx_entries<int>(const int* array, int lda, int sda, bool transpose)
{


    const int *host_pointer;
    
    host_pointer=(const int *)malloc(lda * sda * sizeof(int));
    checkErrors(host_pointer);
    checkCudaErrors(cudaMemcpy((void*)host_pointer, array, lda * sda * sizeof(int),cudaMemcpyDeviceToHost));
...
template<>
void print_gpu_mtx_entries<float>(const float* array, int lda, int sda, bool transpose)
{
    ...
    host_pointer=(const float *)malloc(lda * sda * sizeof(float));
    checkErrors(host_pointer);
    checkCudaErrors(cudaMemcpy((void*)host_pointer, array, lda * sda * sizeof(float),cudaMemcpyDeviceToHost));
...
template<>
void print_gpu_array_entries<int>(const int* array, int count)
{


    const int *host_pointer;
    
    host_pointer=(const int *)malloc(count * sizeof(int));
    checkErrors(host_pointer);
    checkCudaErrors(cudaMemcpy((void*)host_pointer, array, count * sizeof(int),cudaMemcpyDeviceToHost));

    std::string line;
    line="[ ";
     for( int i = 0; i < count; i+= 1 ) {
          if (i==count-1){
            line = (line + ToString(host_pointer[i ])).c_str();
          }else{
            line = (line + ToString(host_pointer[i]) + " , ").c_str();
          };
      };     
...
template<>
void print_gpu_array_entries<float>(const float* array, int count)
{


    const float *host_pointer;
    
    host_pointer=(const float *)malloc(count * sizeof(float));
    checkErrors(host_pointer);
    checkCudaErrors(cudaMemcpy((void*)host_pointer, array, count * sizeof(float),cudaMemcpyDeviceToHost));
...
template <>
void save_device_array_to_file<bool>(const bool* A_dev, int count, std::string title)
{
  ABORT_IF_NEQ(0, 1, "There is an error in this function");
  bool *A_host = NULL;
  A_host=(bool *)malloc(count * sizeof(bool));
  checkErrors(A_host);
  checkCudaErrors(cudaMemcpy(A_host, A_dev, count * sizeof(bool), cudaMemcpyDeviceToHost));
  ...
  //entries<<A_host[i ];
  entries<<(ToString(A_host[i ])).c_str();
  entries<<(int)A_host[i ];
  if(i < count - 1){
    entries<<", ";
  };
...
template <>
void save_device_array_to_file<int>(const int* A_dev, int count, std::string title)
{
...
template <>
void save_device_array_to_file<float>(const float* A_dev, int count, std::string title)
{
...
template <>
void save_device_array_to_file<double>(const double* A_dev, int count, std::string title)
{

  double *A_host = NULL;
  A_host=(double *)malloc(count * sizeof(double));
  checkErrors(A_host);
  checkCudaErrors(cudaMemcpy(A_host, A_dev, count * sizeof(double), cudaMemcpyDeviceToHost));
...
template <>
void append_device_array_to_file<int>(const int* A_dev, int count, std::string title)
{
...
template <>
void append_device_array_to_file<float>(const float* A_dev, int count, std::string title)
{
...
template <>
void append_device_array_to_file<double>(const double* A_dev, int count, std::string title)
{
...
template <>
void save_device_mtx_to_file<int>(const int* A_dev, int lda, int sda, std::string title, bool transpose)
{

  int* A_host = NULL;
  A_host = (int *)malloc(lda*sda* sizeof(int)); 
  checkErrors(A_host);
  checkCudaErrors(cudaMemcpy(A_host, A_dev, lda*sda*sizeof(int), cudaMemcpyDeviceToHost));
...
template <>
void save_device_mtx_to_file<float>(const float* A_dev, int lda, int sda, std::string title, bool transpose)
{

  float* A_host = NULL;
  A_host = (float *)malloc(lda*sda* sizeof(float)); 
  checkErrors(A_host);
  // float A_host[lda*sda];
  CUDA_CHECK(cudaMemcpy(A_host, A_dev, lda*sda*sizeof(float), cudaMemcpyDeviceToHost));
...
template <>
void save_device_mtx_to_file<double>(const double* A_dev, int lda, int sda, std::string title, bool transpose)
{

  double A_host[lda*sda];
  checkCudaErrors(cudaMemcpy(A_host, A_dev, lda*sda*sizeof(double), cudaMemcpyDeviceToHost));
  if(lda==1 && sda==1){
    std::cout<< "num_surviving: "<<A_host[0];
    return;
  };
...
template <>
void gpu_add_scalar<int>(const long long int  N, const int alpha, int* Y) {
  // NOLINT_NEXT_LINE(whitespace/operators)
  int num_gpu_blocks= GET_BLOCKS(N);
  if (num_gpu_blocks > CUDA_NUM_BLOCKS){
    ABORT_IF_NEQ(0, 1, "Max Blocks Exceeded");
  }; 
  add_scalar_kernel<int><<<GET_BLOCKS(N), CUDA_NUM_THREADS>>>(
      N, alpha, Y);
}

template <>
void gpu_add_scalar<float>(const long long int  N, const float alpha, float* Y) {
  // NOLINT_NEXT_LINE(whitespace/operators)
  ...
  if (num_gpu_blocks > CUDA_NUM_BLOCKS){
    ...
    while (num_gpu_blocks > CUDA_NUM_BLOCKS){
      add_scalar_kernel<float><<<CUDA_NUM_BLOCKS, CUDA_NUM_THREADS>>>(num_entries, alpha, Y + spot);
    ...
    // spot is the number of entries done so far
    // total - (done) = left to go 
    add_scalar_kernel<float><<<num_gpu_blocks, CUDA_NUM_THREADS>>>(N - spot, alpha, Y + spot);
  }else{
    add_scalar_kernel<float><<<num_gpu_blocks, CUDA_NUM_THREADS>>>(N, alpha, Y);
  };
}
...
template <>
void gpu_add_scalar<double>(const long long int  N, const double alpha, double* Y) {
  // NOLINT_NEXT_LINE(whitespace/operators)
  int num_gpu_blocks= GET_BLOCKS(N);
  if (num_gpu_blocks > CUDA_NUM_BLOCKS){
    ABORT_IF_NEQ(0, 1, "Max Blocks Exceeded");
...
template <>
void gpu_get_rand_bools<bool>(const int n,  bool* x, unsigned long seed, float probability_of_success) 
{
    curandState *devState;  
    gpuErrchk(cudaMalloc((void**)&devState, n*sizeof(curandState)));

    const int num_gpu_blocks= GET_BLOCKS(n);
    ...
    gpu_get_rand_bools_kernel<bool><<<num_gpu_blocks, CUDA_NUM_THREADS>>>(devState, x, n, probability_of_success);
    gpuErrchk(cudaPeekAtLastError());
    gpuErrchk(cudaDeviceSynchronize());
...
template <>
void gpu_get_rand_bools<float>(const int n,  float* x, unsigned long seed, float probability_of_success) 
{
    curandState *devState;  
    gpuErrchk(cudaMalloc((void**)&devState, n*sizeof(curandState)));
    ...
    gpu_get_rand_bools_kernel<float><<<num_gpu_blocks, CUDA_NUM_THREADS>>>(devState, x, n, probability_of_success);
    gpuErrchk(cudaPeekAtLastError());
    gpuErrchk(cudaDeviceSynchronize());
...
void gpu_get_rand_groups(const int n,  int* x, unsigned long seed, float* probability_of_success, const int num_groups) 
{
    curandState *devState;  
    gpuErrchk(cudaMalloc((void**)&devState, n*sizeof(curandState)));
...
template <>
void gpu_shuffle_array<float>(cublasHandle_t handle, const int n,  float* x)
{
    float* order;
...
template <>
void gpu_shuffle_array<int>(cublasHandle_t handle, const int n,  int* x)
{
    float* order;
    CUDA_CHECK(cudaMalloc((void**)&order, n* sizeof(float)));
    ...
    // print_gpu_array_entries<int>(order, 10 , 1 , n);
    // print_gpu_array_entries<int>(x, 10 , 1 , n);

    thrust::device_ptr<float> order_(order);
    thrust::device_vector<float> order__(order_, order_ + n);
    thrust::device_ptr<int> r_(x);
    thrust::device_vector<int> r__(r_, r_ + n);
    ...
    // print_gpu_array_entries<int>(order, 10 , 1 , n);
    // print_gpu_array_entries<int>(r, 10 , 1 , n);

    cudaFree(order);

    // order__.clear();
    // r__.clear();
    // thrust::device_vector<int>().swap(order__);
    // thrust::device_vector<int>().swap(r__);
}
...
template<>
void gpu_set_all<int>(  int* x, const int n, const int alpha) 
{
  int num_gpu_blocks= GET_BLOCKS(n);
  if (num_gpu_blocks > CUDA_NUM_BLOCKS){
    ABORT_IF_NEQ(0, 1, "Max Blocks Exceeded");
  }; 
   gpu_set_all_kernel<int><<<num_gpu_blocks, CUDA_NUM_THREADS>>>( x, n, alpha);

}
template<>
void gpu_set_all<float>(  float* x, const int n, const float alpha) 
{
  int num_gpu_blocks= GET_BLOCKS(n);
  // LOG(INFO) << "num_gpu_blocks: "<<num_gpu_blocks;
  ...
  if (num_gpu_blocks > CUDA_NUM_BLOCKS){
    ...
    while (num_gpu_blocks > CUDA_NUM_BLOCKS){
      // LOG(INFO) << "spot: "<<spot;
      gpu_set_all_kernel<float><<<CUDA_NUM_BLOCKS, CUDA_NUM_THREADS>>>( x + spot, num_entries, alpha);



      // LOG(INFO) << "x: ";
      // print_gpu_array_entries<float>(x, x_rows , x_cols , x_rows);
    ...
    gpu_set_all_kernel<float><<<num_gpu_blocks, CUDA_NUM_THREADS>>>( x + spot, n - spot, alpha);
  }else{
    gpu_set_all_kernel<float><<<num_gpu_blocks, CUDA_NUM_THREADS>>>( x, n, alpha);
  };
}
template<>
void gpu_set_all<double>(  double* x, const int n, const double alpha) 
{
  int num_gpu_blocks= GET_BLOCKS(n);
  if (num_gpu_blocks > CUDA_NUM_BLOCKS){
    ABORT_IF_NEQ(0, 1, "Max Blocks Exceeded");
  }; 
   gpu_set_all_kernel<double><<<num_gpu_blocks, CUDA_NUM_THREADS>>>( x, n, alpha);
...
void gpu_set_as_index(  int* x, const int n) 
{
  int num_gpu_blocks= GET_BLOCKS(n);
  if (num_gpu_blocks > CUDA_NUM_BLOCKS){
    ABORT_IF_NEQ(0, 1, "Max Blocks Exceeded");
...
template <>
int gpu_abs_max<int>(const int n,  const int* x) 
{
...
template <>
float gpu_abs_max<float>(const int n,  const float* x) 
{
  // int num_gpu_blocks = GET_BLOCKS(n);
  // float* d_max_dev;
  // CUDA_CHECK(cudaMalloc((void**)&d_max_dev, sizeof(float)));

  // abs_max_reduce<float><<<num_gpu_blocks, CUDA_NUM_THREADS>>>(x, d_max_dev, n); 

  // float *d_max = (float* ) malloc(sizeof(float));

  // CUDA_CHECK(cudaMemcpy(d_max, d_max_dev, sizeof(float), cudaMemcpyDeviceToHost));
  // cudaFree(d_max_dev);
  // float max = d_max[0];
  // free(d_max);
  // LOG(INFO) << "max : "<<max;
...
template <>
double gpu_abs_max<double>(const int n,  const double* x) 
{
  double* y = (double*) x;
  //thrust::device_ptr<float> X = thrust::device_pointer_cast(y);
  //int data[6] = {1, 0, 2, 2, 1, 3};
  thrust::pair<double *, double *> result = thrust::minmax_element(y, y + n);
  // result.first is data + 1
  // result.second is data + 5
  // *result.first is 0
  // *result.second is 3
...
template <>
float gpu_min<float>(const int n,  const float* x) 
{
...
template <>
float gpu_max<float>(const int n,  const float* x) 
{
...
template <>
float gpu_range<float>(const int n,  const float* x) 
{
...
template <>
float gpu_sum<float>(const int n,  const float* x) 
{
  float* y = (float*) x;
  thrust::device_ptr<float> X = thrust::device_pointer_cast(y);
...
template <>
float gpu_expected_value<float>(const int n,  const float* x) 
{
  float* y = (float*) x;
  thrust::device_ptr<float> X = thrust::device_pointer_cast(y);
  float sum = thrust::reduce(X, X + n, (float)0., thrust::plus<float>());
...
template <>
double gpu_expected_value<double>(const int n, const double* x) 
{
  double* y = (double*) x;
  thrust::device_ptr<double> X = thrust::device_pointer_cast(y);
  double sum = thrust::reduce(X, X + n, (double)0., thrust::plus<double>());
...
template <>
float gpu_expected_abs_value<float>(const int n,  const float* x) {
  // setup arguments 
  abss<float> unary_op; 
...
template <>
float gpu_variance<float>(const int n, const float* x) {
...
template <>
float gpu_sum_of_squares<float>(const int n, const float* x) {
  square<float> unary_op; 
  thrust::plus<float> binary_op; 
  float init = 0; 
  // compute norm 
...
template <>
void gpu_dot<float>(cublasHandle_t dn_handle, const int n, const float* x, const float* y,
    float* out) {
  CUBLAS_CHECK(cublasSdot(dn_handle, n, x, 1, y, 1, out));
}

template <>
void gpu_dot<double>(cublasHandle_t dn_handle, const int n, const double* x, const double* y,
    double * out) {
  CUBLAS_CHECK(cublasDdot(dn_handle, n, x, 1, y, 1, out));
}

template <>
void gpu_asum<float>(cublasHandle_t dn_handle, const int n, const float* x, float* y) {
  CUBLAS_CHECK(cublasSasum(dn_handle, n, x, 1, y));
}

template <>
void gpu_asum<double>(cublasHandle_t dn_handle, const int n, const double* x, double* y) {
...
cublasStatus_t transpose<float>(cublasHandle_t handle, 
                                int m, int n, const float *A, float *result)
{
    float one = 1;
    float zero = 0;
...
cublasStatus_t transpose<double>(cublasHandle_t handle,
                                       int m, int n, const double *A, double *result)
{
    double one = 1;
    double zero = 0;
    cublasOperation_t trans = CUBLAS_OP_T;
...
template<>
void gpu_hadamard<float>(const int n, const float* A, float* B ) 
{
  //performs B = A * B
  int num_gpu_blocks= GET_BLOCKS(n);
  // if (num_gpu_blocks > max_blocks){
  //   CHECK_EQ(0, 1) << "Max Blocks Exceeded";
  // };
...
template <>
void gpu_axpy<float>(cublasHandle_t dn_handle, const int N, const float alpha, const float* X,
                           float* Y) 
{
...
template <>
void gpu_axpy<double>(cublasHandle_t dn_handle, const int N, const double alpha, const double* X,
                            double* Y) 
{
...
template <>
void gpu_axpby<float>(cublasHandle_t dn_handle, const int N, const float alpha, const float* X,
                            const float beta, float* Y) {
  gpu_scale<float>(dn_handle, N, beta, Y);
  gpu_axpy<float>(dn_handle, N, alpha, X, Y);
      /*
       cublasSaxpy multiplies the vector x by the scalar α and adds it
...
template <>
void gpu_axpby<double>(cublasHandle_t dn_handle, const int N, const double alpha, const double* X,
                             const double beta, double* Y) {
  gpu_scale<double>(dn_handle, N, beta, Y);
  gpu_axpy<double>(dn_handle, N, alpha, X, Y);
}

template <>
void gpu_axpby<float>(cublasHandle_t dn_handle, const int N, const float alpha, const float* X,
                            const float beta, const float* Y, float* result) {

  CUDA_CHECK(cudaMemcpy(result, Y, N * sizeof(float), cudaMemcpyDeviceToDevice));
  
  gpu_scale<float>(dn_handle, N, beta, result);
  gpu_axpy<float>(dn_handle, N, alpha, X, result);
      /*
       cublasSaxpy multiplies the vector x by the scalar α and adds it
       to the vector beta * y and puts this value in result. 
       Hence, the performed operation is result [ j ] = α × x [ k ] + beta * y [ j ] 
       for i = 1 , … , n , k = 1 + ( i - 1 ) *  incx and j = 1 + ( i - 1 ) *  incy . 
       Notice that the last two equations reflect 1-based indexing used 
       for compatibility with Fortran.
    */
...
template<>
float gpu_sum_of_squares_of_diff<float>(cublasHandle_t dn_handle, const int n, const float* x, float* y)
{
                        ...
                        const int ratings_rows_testing) 
{
    ...
    const int num_gpu_blocks= GET_BLOCKS(ratings_rows);
                            ...
                            float* full_training_ratings_mtx)
{
  ...
  const int num_gpu_blocks= GET_BLOCKS(ratings_rows_training);
                                                    ...
                                                    const int* coo_format_keyWordMtx_keyWord_dev)
{
  ...
  /*if(Debug)*/ LOG("content based run time : "<<program_time<<"ms");
}


__global__ void center_ratings_kernel(const float* user_means_testing, const float* user_var_testing, const int ratings_rows_testing,
                                        const int* csr_format_ratingsMtx_userID_dev_testing,
                                        const float* coo_format_ratingsMtx_rating_dev_testing,
                                        ...
                                        const float val_when_var_is_zero, bool* isBad) 
{
    ...
    CUDA_KERNEL_LOOP(row, ratings_rows_testing){
        for(int i = csr_format_ratingsMtx_userID_dev_testing[row]; i < csr_format_ratingsMtx_userID_dev_testing[row + 1]; i++){
            float val = coo_format_ratingsMtx_rating_dev_testing[i] - user_means_testing[row];
            if(user_var_testing[row] > (float)0.0){
              val = val / std::sqrt(user_var_testing[row]);
            }else{
              val = val_when_var_is_zero;
            }

            coo_format_ratingsMtx_row_centered_rating_dev_testing[i] = val;
            if (::isinf(val) || ::isnan(val)){
              isBad[0] = true;
            };
        }
...
void center_ratings(const float* user_means_testing, const float* user_var_testing, 
                    const int ratings_rows_testing, const int num_entries_testing,
                    const int* csr_format_ratingsMtx_userID_dev_testing,
                    const float* coo_format_ratingsMtx_rating_dev_testing,
                    float* coo_format_ratingsMtx_row_centered_rating_dev_testing,
                    const float val_when_var_is_zero)
{
  ...
  // max_ - 
    ...
    const int num_gpu_blocks= GET_BLOCKS(ratings_rows_testing);

    if (num_gpu_blocks > CUDA_NUM_BLOCKS){
        ABORT_IF_NEQ(0, 1, "Max Blocks Exceeded");
    };

    center_ratings_kernel<<<num_gpu_blocks, CUDA_NUM_THREADS>>>(user_means_testing, user_var_testing, ratings_rows_testing,
                                                                    csr_format_ratingsMtx_userID_dev_testing,
                                                                    coo_format_ratingsMtx_rating_dev_testing,
                                                                    coo_format_ratingsMtx_row_centered_rating_dev_testing,
                                                                    val_when_var_is_zero, isBad);

  if(Debug){
    float _min = gpu_min<float>           (num_entries_testing, coo_format_ratingsMtx_row_centered_rating_dev_testing);
    float _max = gpu_max<float>           (num_entries_testing, coo_format_ratingsMtx_row_centered_rating_dev_testing);
    float expt = gpu_expected_value<float>(num_entries_testing, coo_format_ratingsMtx_row_centered_rating_dev_testing);
    LOG("_min :"  <<_min);
    LOG("_max :"  <<_max);
    LOG("expt :"  <<expt);
  };
...
template <>
void gpu_permute<float>(float* A, const int* P, int rows, int cols, bool permute_rows) 
{
  /*
  ...
  */
  ...
  if (num_gpu_blocks > CUDA_NUM_BLOCKS){
    ...
    int spot = 0;
    while (num_gpu_blocks > CUDA_NUM_BLOCKS){
      gpu_permute_kernel<float><<<CUDA_NUM_BLOCKS, CUDA_NUM_THREADS>>>(A,  P, rows, cols,  permute_rows, spot, num_entries);
      num_gpu_blocks= num_gpu_blocks - CUDA_NUM_BLOCKS;
      num_loops+=1;
      spot = num_loops * num_entries;
    };
    gpu_permute_kernel<float><<<num_gpu_blocks, CUDA_NUM_THREADS>>>(A,  P, rows, cols,  permute_rows, spot, permute_rows ? (cols - spot) : (rows - spot));
  }else{
    gpu_permute_kernel<float><<<num_gpu_blocks, CUDA_NUM_THREADS>>>(A,  P, rows, cols,  permute_rows, 0, permute_rows ? cols : rows);
  };
}

template <>
void gpu_permute<double>(double* A, const int* P, int rows, int cols, bool permute_rows) 
{
  /*
  This function applies a permutation mtx P (rows x rows) to the left of mtx a (rows x cols)
  with the ones in P are located at indices indicated by pvt which is an array with length rows
  ...
  */
  ...
  if (num_gpu_blocks > CUDA_NUM_BLOCKS){
    ...
    int spot = 0;
    while (num_gpu_blocks > CUDA_NUM_BLOCKS){
      gpu_permute_kernel<double><<<CUDA_NUM_BLOCKS, CUDA_NUM_THREADS>>>(A,  P, rows, cols,  permute_rows, spot, num_entries);
      num_gpu_blocks= num_gpu_blocks - CUDA_NUM_BLOCKS;
      num_loops+=1;
      spot = num_loops * num_entries;
    };
    gpu_permute_kernel<double><<<num_gpu_blocks, CUDA_NUM_THREADS>>>(A,  P, rows, cols,  permute_rows, spot, permute_rows ? (cols - spot) : (rows - spot));
  }else{
    gpu_permute_kernel<double><<<num_gpu_blocks, CUDA_NUM_THREADS>>>(A,  P, rows, cols,  permute_rows, 0, permute_rows ? cols : rows);
  };
}

void gpu_shuffle_mtx_rows_or_cols(cublasHandle_t dn_handle, const int M, const int N,  
...
void gpu_copy_and_transpose(const int M, const int N,  const float* x, float* y) 
{
    ...
    const int num_gpu_blocks= GET_BLOCKS(M * N);
                                  ...
                                  float* A, float* U, float* V) 
{
  ...
  */
    ...
    program_time = (program_end.tv_sec * 1000 +(program_end.tv_usec/1000.0))-(program_start.tv_sec * 1000 +(program_start.tv_usec/1000.0));
    //printf("program_time: %f\n", program_time);   
    /*if(Debug)*/ LOG("gpu_orthogonal_decomp run time : "<<program_time<<"ms");
}                    
                  ...
                  float* coo_A) 
{
  ...
  */
  ...
  const int num_gpu_blocks= GET_BLOCKS(num_sparse_entries);
                              ...
                              const float *B, const int ldb, const float *beta, float *C, const int ldc)
{
  ...
  */
  ...
  if(Debug){
    ...
    save_device_array_to_file<int>(csrRowPtrA_temp, m + 1, "csrRowPtrA_temp");
...
void gpu_R_error<float>(const cublasHandle_t dn_handle, const cusparseHandle_t sp_handle, const cusparseMatDescr_t sp_descr,
                        const int batch_size_testing, const int batch_size_GA, const int num_latent_factors, const int ratings_cols,
                        ...
                        std::string name, float training_rate, float regularization_constant)
{
  ...
  /*
      ...
      V is dense ratings_cols by batch_size_GA (or num_latent_factors)
  ...
  */
  ...
  if(Debug && 0 ){
    ...
    //CUDA_CHECK( cudaMalloc( (void**)&U_testing_check, batch_size_testing * batch_size_GA  * sizeof(float) ) );
    LOG("gpu_spXdense_MMM<float>(handle, TransA = "<<false<<" , TransB = "<< false<<","<<std::endl<<"                                                "
                                <<"m = "<<batch_size_testing<<" , n = "<<batch_size_GA<<" , k = "<<ratings_cols<<","<<std::endl<<"                                                "
                                <<"nnz = "<<  nnz<<" , first_coo_ind = "<<  first_coo_ind<<","<<std::endl<<"                                                "
                                <<"&alpha, sp_descr,"<<std::endl<<"                                                "
                                <<"testing_entries,csr_format_ratingsMtx_userID_dev_testing_batch, coo_format_ratingsMtx_itemID_dev_testing,"<<std::endl<<"                                                "
                                <<"V, ldb = "<<ratings_cols<<", &beta,"<<std::endl<<"                                                "
    ...
    //                              (compress == false) ? batch_size_GA : num_latent_factors, 
  ...
  gpu_spXdense_MMM<float>(sp_handle, false, false, batch_size_testing,
                           ...
                           csr_format_ratingsMtx_userID_dev_testing_batch, 
                           coo_format_ratingsMtx_itemID_dev_testing,
                           V, ratings_cols, &beta, U_testing, batch_size_testing);


  // V^T * V
  // gpu_gemm<float>(dn_handle, false, true, (compress == false) ? batch_size_GA : num_latent_factors,
  //                 (compress == false) ? batch_size_GA : num_latent_factors, ratings_cols, 
  //                 (float)1.0, V, V, (float)0.0, );
  ...
  */
  ...
  if(Debug && 0){
    if(compress){
      ...
      save_device_mtx_to_file<float>(U_testing, batch_size_testing, batch_size_GA, "U_testing");
      save_device_mtx_to_file<float>(V, ratings_cols, batch_size_GA, "V");
      //save_device_arrays_side_by_side_to_file<float>(U_testing, U_testing_check, batch_size_testing * batch_size_GA     , "U_testing");
    }
  ...
  //for(int i = 0; i < 5; i++){
    ...
    if( Debug){
      ...
      std::string title = ("ratings_testing_errors_v" + ToString(i)).c_str();
  ...
  program_time = (program_end.tv_sec * 1000 +(program_end.tv_usec/1000.0))-(program_start.tv_sec * 1000 +(program_start.tv_usec/1000.0));
  //printf("program_time: %f\n", program_time);   
  if(1) LOG((title + " run time : ").c_str()<<program_time<<"ms");
}
...
void gpu_R_error_training<float>(const cublasHandle_t dn_handle, const cusparseHandle_t sp_handle, const cusparseMatDescr_t sp_descr,
                                const int batch_size_training, const int batch_size_GA, const int num_latent_factors, const int ratings_cols,
                                ...
                                float training_rate, float regularization_constant)
{
  ...
  */
  ...
  if(Debug && 0 ){
    ...
    //CUDA_CHECK( cudaMalloc( (void**)&U_training_check, batch_size_training * batch_size_GA  * sizeof(float) ) );
    LOG("gpu_spXdense_MMM<float>(handle, TransA = "<<false<<" , TransB = "<< false<<","<<std::endl<<"                                                "
                                <<"m = "<<batch_size_training<<" , n = "<<batch_size_GA<<" , k = "<<ratings_cols<<","<<std::endl<<"                                                "
                                <<"nnz = "<<  nnz<<" , first_coo_ind = "<<  first_coo_ind<<","<<std::endl<<"                                                "
                                <<"&alpha, sp_descr,"<<std::endl<<"                                                "
                                <<"coo_format_ratingsMtx_rating_dev_training,csr_format_ratingsMtx_userID_dev_training_batch, coo_format_ratingsMtx_itemID_dev_training,"<<std::endl<<"                                                "
                                <<"V, ldb = "<<ratings_cols<<", &beta,"<<std::endl<<"                                                "
  ...
  gpu_spXdense_MMM<float>(sp_handle, false, false, batch_size_training,
                           ...
                           csr_format_ratingsMtx_userID_dev_training_batch, 
                           coo_format_ratingsMtx_itemID_dev_training,
                           V, ratings_cols, &beta, U_training, batch_size_training);
  ...
  */
  ...
  if(Debug && 0){
    if(compress){
      ...
      save_device_mtx_to_file<float>(U_training, batch_size_training, batch_size_GA, "U_training");
      save_device_mtx_to_file<float>(V, ratings_cols, batch_size_GA, "V");
      //save_device_arrays_side_by_side_to_file<float>(U_training, U_training_check, batch_size_training * batch_size_GA     , "U_training");
    }
  }
  ...
  //if(Debug) cudaFree(U_training_check);
  cudaDeviceSynchronize();
  gettimeofday(&program_end, NULL);
  program_time = (program_end.tv_sec * 1000 +(program_end.tv_usec/1000.0))-(program_start.tv_sec * 1000 +(program_start.tv_usec/1000.0));
  //printf("program_time: %f\n", program_time);   
  if(1) LOG("gpu_R_error_training run time : "<<program_time<<"ms");
}
                        ...
                        float training_rate, float regularization_constant)
{
  ...
  */
  ...
  if(1) {
    ...
    LOG("gpu_R_error_testing error : "<<error);     
  }
  ...
  program_time = (program_end.tv_sec * 1000 +(program_end.tv_usec/1000.0))-(program_start.tv_sec * 1000 +(program_start.tv_usec/1000.0));
  //printf("program_time: %f\n", program_time);   
  if(1) LOG("gpu_R_error_testing run time : "<<program_time<<"ms");
}
